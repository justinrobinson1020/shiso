...
# Order does not matter (spec §4 step 6); files are posted in sorted order. CURL_OPTS adds flags, e.g. -k for the homelab's self-signed CA. The Origin header is required: SvelteKit rejects form posts without one.
set -euo pipefail
