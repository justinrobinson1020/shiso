...
Balances are from the account's point of view: a card's new balance of $686.91 is stored as −68691, matching opening rows.
...
- Requesting the Budget page for a period before `budget_start` renders the header and a notice ("Before the budget started on Jun 1, 2026. History only.") instead of the envelope grid; assignment and funding routes reject such periods with 422.
