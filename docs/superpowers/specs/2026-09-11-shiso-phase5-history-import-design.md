...
**Reconciliation** (`statement !== null`): `previousBalance + Σ row.amount = newBalance` to the cent. Failure reports the signed difference (`previousBalance + Σ rows − newBalance`, so the direction is readable across a long run) and rejects the file. A Chase statement with no `ACCOUNT ACTIVITY` block (a zero-balance month) has zero rows and reconciles when both balances are zero.
...
5. **Balances.** Each `parsed.balances` entry is appended with `source = 'import'` unless an import balance with the same `asOf` and `current` already exists on the account (the sheet importer's `hasBalance`), or the account has a synced balance dated on or after it — a statement sample that ties or beats the newest sync would outrank it in `latestBalance` and move ready-to-assign. Counted `balances`.
6. **Opening balance.** Let `O` be the account's live `source = 'opening'` row, if any, `S = parsed.statement`, and `before(S)` the sum of the live non-opening rows dated before `S.opensOn` once step 4's rows exist. A statement states the account's balance the day before it opens, so the plug is what that balance is not already explained by.
   - **`S` present.** The opening day is `opensOn − 1` and the plug is `S.previousBalance − before(S)`.
     - `O` missing: seed it there (`externalId = 'opening'`, `source = 'opening'`, reconciliation category), reported as `opening: { seeded, date }`. If the id is already owned by a soft-deleted opening row (`DUPLICATE_EXTERNAL_ID`), skip the seed and report `null`.
     - `O` dated on or after `opensOn − 1`: this is the earliest history the account has, so `O` moves to that day and that amount (its single reconciliation split and its `periodId` follow), reported as `opening: { from, to, date }` when either actually changed and `null` when neither did.
     - `O` dated before `opensOn − 1`: it already covers older history. Untouched.
   - **`S` absent (CSV).** If any row created in step 4 is dated on or before `O.postedDate`: `O.amount −= Σ` of those rows, `O.postedDate = min(their dates) − 1 day`, split and `periodId` follow. Reported as `opening: { from, to, date }`.
   - The opening row is marked processed, as `sync/apply.ts` does, so transfer detection never links it.
   - Idempotence: a re-import recomputes the same amount and date, so it reports `null` and writes nothing. Order independence: the plug is derived from the earliest statement seen, not from the order they arrive in, so both orders leave the same `O` and the same ledger sum.
   - Whenever `S` is present the report carries two deltas, both computed after the opening step and `null` without a statement. `previousDelta`: the sum of every live row (`O` included) dated before `opensOn`, minus `S.previousBalance` — 0 when history up to this statement is complete, nonzero while a gap remains, which understates the ledger by exactly the missing rows. `closingDelta`: the same sum through `closesOn`, minus `S.newBalance`. Reconciliation already ties the file's own rows together, so the two deltas differ by exactly what step 4 got wrong inside this statement's window — a row matched against a synced transaction it is not, or one that should have matched and did not. Both are reported and neither is ever rejected on.
...
{ format, mask, statement: { opensOn, closesOn, previousBalance, newBalance } | null,
  ...
  previousDelta: number | null, closingDelta: number | null,
...
4. `scripts/import-history.sh <dir> <map-file> (--dry-run|--commit)` — the map file lists `folder-or-file  accountId` pairs; the script posts every file under each folder to `$SHISO_URL/api/accounts/<id>/import` in directory order (order does not matter, §4 step 6), printing one report line per file, a `gap:` line on stderr for a nonzero `previousDelta` or `closingDelta`, and stopping on the first non-2xx. The mode is required; through npm it needs npm's separator (`npm run import:history -- <dir> <map> --dry-run`), which npm would otherwise eat. Run `--dry-run` first; every file must reconcile and the created/matched counts must look right (overlap months should be mostly `matched`). Then `--commit`.
